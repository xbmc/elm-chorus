...
{
	if (region.al.N === region.ax.N)
	{
		return 'on line ' + region.al.N;
	}
	return 'on lines ' + region.al.N + ' through ' + region.ax.N;
}
...
var $author$project$Path$optionalParams = A2(
	...
	function (num) {
		return _Utils_Tuple2(
			'param' + $elm$core$String$fromInt(num),
			'String');
	});
var $author$project$Path$toFlags = function (path) {
...
var $author$project$Add$Component$create = function (path) {
	return $elm$core$String$trim(
		A3(
			$elm$core$String$replace,
			'{{flags}}',
			$author$project$Path$toFlags(path),
			A3(
				$elm$core$String$replace,
				'{{name}}',
...
var $author$project$Ports$add = function (_v0) {
	...
	var sendItBro = function (content) {
		return $author$project$Ports$addPort(
			{
				L: content,
				M: $author$project$Path$toFilepath(path)
			});
	};
	switch (pageType) {
		case 'static':
		...
		case 'component':
			return sendItBro(
				$author$project$Add$Component$create(path));
		default:
			...
			_List_fromArray(
				[
					...
					A2(
						$elm$core$String$join,
						'\n\n',
						A2($elm$core$List$map, toBranch, options.aa)))
				]));
	}
};
var $author$project$Utils$Generate$function = function (options) {
	...
	if (!_v0.b) {
		...
		return A2(
			...
			_List_fromArray(
				[
					options.W + (' : ' + A2($elm$core$String$join, ' -> ', options.Z)),
					options.W + (' ' + (A3(
					$elm$core$List$foldl,
					F2(
						function (arg, str) {
					...
					'',
					options.ae) + '=')),
					A2($author$project$Utils$Generate$indent, 1, options._)
				]));
	}
};
var $author$project$Path$toTypeName = $author$project$Path$join('_');
...
var $author$project$Path$toVariableName = function (_v0) {
	...
	return A2(
		$elm$core$String$join,
		'_',
		A2($elm$core$List$map, lowercaseFirstLetter, list));
};
var $author$project$Generators$Pages$pagesBundle = function (paths) {
	return $author$project$Utils$Generate$function(
		{
			Z: _List_fromArray(
				['Model', 'Global.Model', 'Bundle Msg']),
			_: $author$project$Utils$Generate$caseExpression(
				{
					aa: A2(
						$elm$core$List$map,
						function (path) {
...
var $author$project$Generators$Pages$pagesInit = function (paths) {
	return $author$project$Utils$Generate$function(
		{
			Z: _List_fromArray(
				['Route', 'Global.Model', '( Model, Cmd Msg, Cmd Global.Msg )']),
			_: $author$project$Utils$Generate$caseExpression(
				{
					aa: A2(
						$elm$core$List$map,
		...
		$elm$core$List$concat(
			_List_fromArray(
				[
					_List_fromArray(
					[
						_Utils_ap(prefixes.n, items.n)
					]),
					A2(
					$elm$core$List$map,
					$elm$core$String$append(prefixes.p),
					items.p),
					suffix
				])));
};
		...
		return $author$project$Utils$Generate$customType(
			{
				...
				bH: A2(
					$elm$core$List$map,
					function (path) {
						return $author$project$Path$toTypeName(path) + ('_' + (name + (' Pages.' + ($author$project$Path$toModulePath(path) + ('.' + name)))));
					},
					paths)
			});
	});
...
var $author$project$Generators$Pages$pagesUpdate = function (paths) {
	return $author$project$Utils$Generate$function(
		{
			Z: _List_fromArray(
				['Msg', 'Model', 'Global.Model', '( Model, Cmd Msg, Cmd Global.Msg )']),
			_: $author$project$Utils$Generate$caseExpression(
				{
					aa: function (cases) {
						return ($elm$core$List$length(paths) === 1) ? cases : _Utils_ap(
							...
							_List_fromArray(
								[
									_Utils_Tuple2('_', 'always ( bigModel, Cmd.none, Cmd.none )')
								]));
					}(
						A2(
							$elm$core$List$map,
							function (path) {
								var typeName = $author$project$Path$toTypeName(path);
								return _Utils_Tuple2(
									'( ' + (typeName + ('_Msg msg, ' + (typeName + '_Model model )'))),
				...
				return $author$project$Utils$Generate$multilineThing(
					{
						F: {
							n: fromTuple(first),
							p: A2($elm$core$List$map, fromTuple, rest)
						},
						G: {n: '{ ', p: ', '},
						I: _List_fromArray(
							['}'])
			...
			A2(
				...
				function (path) {
					...
					return _Utils_Tuple2(
						...
						'UpgradedPage ' + (name + ('.Flags ' + (name + ('.Model ' + (name + '.Msg'))))));
				},
				paths)));
};
		...
		$author$project$Utils$Generate$recordValue(
			A2(
				...
				function (path) {
					return _Utils_Tuple2(
						$author$project$Path$toVariableName(path),
						'Pages.' + ($author$project$Path$toModulePath(path) + ('.page |> Page.upgrade ' + ($author$project$Path$toTypeName(path) + ('_Model ' + ($author$project$Path$toTypeName(path) + '_Msg'))))));
				},
				paths)));
};
...
var $author$project$Path$toParser = function (_v0) {
	...
	var toUrlSegment = function (piece) {
		return (piece === 'Dynamic') ? 'Parser.string' : ('Parser.s \"' + ($author$project$Path$sluggify(piece) + '\"'));
	};
	...
	var dynamicParamsFn = function (list_) {
		return '\\' + (A2(
			...
			A2(
				$elm$core$List$map,
				function (num) {
					return 'param' + $elm$core$String$fromInt(num);
				},
				A2($elm$core$List$range, 1, count))) + (' ->' + toParamMap(list_)));
	};
...
var $author$project$Path$toParamList = function (_v0) {
	...
	var helper = F2(
		function (piece, _v1) {
			var names = _v1.a;
			var num = _v1.b;
			return (piece === 'Dynamic') ? _Utils_Tuple2(
				_Utils_ap(
					names,
					_List_fromArray(
						[
	...
	return function (items) {
		...
		A3(
			...
			$author$project$Path$stripEndingTop(list)).a);
};
...
var $author$project$Ports$build = function (paths) {
	return $author$project$Ports$buildPort(
		_List_fromArray(
			[
				{
				L: $author$project$Generators$Route$generate(paths),
				M: 'Generated/Route.elm'
			},
				{
				L: $author$project$Generators$Pages$generate(paths),
				M: 'Generated/Pages.elm'
			}
			]));
};
...
var $author$project$Path$routingOrder = F2(
	function (_v0, _v1) {
		...
		var endsInDynamic = endsIn('Dynamic');
		var endsInTop = endsIn('Top');
		return (_Utils_cmp(
							...
							return A2(
								...
								function (command) {
									return $elm$json$Json$Decode$succeed(
										{au: command, az: filepaths, W: name, ai: pageType});
								},
